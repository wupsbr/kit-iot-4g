...
KitIoT.prototype.start = function () {
  ...
  self.loop = setInterval(doSaveData, 60000);
  self.loopDash = setInterval(doUpdateDashboard, 2000);
};
...
KitIoT.prototype.getSensorValues = function () {
  return {
    ...
    temperature: (( 5.0 * parseFloat(this.temperature.value || 0) * 100.0) / 1024.0).toFixed(1), // to celsius
    capacitive : this.capacitive.value
  };
};
